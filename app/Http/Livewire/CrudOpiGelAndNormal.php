...
class CrudOpiGelAndNormal extends Component
{
    ...
    public function deleteSelected(){
        OpiGelAndNormal::destroy($this->checkedItems);
    ...
    public function editData($id){
        $this->emptyData();
        $item = OpiGelAndNormal::find($id)->toArray();
        $this->modelData = $item;
        $this->menuOpen = true;
    }
    ...
    public function submit(){
        if(isset($this->modelData['id'])){
            OpiGelAndNormal::find($this->modelData['id'])->update($this->modelData);
            $this->dispatchBrowserEvent('from-backend',['is'=>'toastr','type'=>'success','message'=>'Item Updated Successfully']);
        }
        else{
            OpiGelAndNormal::create($this->modelData);
    ...
    {
        $this->items = OpiGelAndNormal::orderBy('id','desc')->get();
        return view('livewire.crud-opi-gel-and-normal');
    }
}
